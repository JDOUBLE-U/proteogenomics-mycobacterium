...
def save_hit(cleavage_loc, motif_len, prot, protein_db, weblogo_in_file, readable_out_file, complete_fasta_out_file,
             maxquant_csv_writer):
...
def find_metap_activity(min_prob, cleavage_loc, motif_len, prots, protein_db, weblogo_in_path, complete_fasta_out_path,
		...
		for prot in matching_prot_hits:
			for pep in prot.get_peptides():
				...
				if pep_pos == cleavage_loc:
					...
					save_hit(cleavage_loc, motif_len, prot, protein_db, weblogo_in_file, readable_out_file, complete_fasta_out_file,
					         maxquant_csv_writer)
...
def run_metap_pipeline(ms_run_code, prot_prophet_xml, protein_db_path, min_prob, cleavage_loc,
	...
	complete_fasta_out_name = parse_results_out_paht + protxml_name + ms_run_code + '_complete.fasta'
